...
3. **Profundidad por tono y borde, nunca por sombra.** Solo los elementos flotantes (menús, popovers, toasts) llevan la _Technical Shadow_. Cards, paneles y tablas: `1px` de borde o cambio de superficie.
...
Contrastes verificados: 6.6:1 · 6.3:1 · 6.5:1 sobre blanco; 13.0:1 · 13.2:1 · 7.2:1 en variante _container_.
...
- El atributo `data-theme` lo estampa un **script inline en `index.html` antes de pintar**, para no producir _flash_ de tema claro.
...
| `title-sm` _(añadido)_ | Montserrat 600     | 18 / 24          | —        | Título de bloque dentro del editor                  |
| `body-lg`              | Hanken Grotesk 400 | 18 / 28          | —        | Texto introductorio                                 |
| `body-md`              | Hanken Grotesk 400 | 16 / 24          | —        | **Cuerpo por defecto** y valor de input             |
| `body-sm` _(añadido)_  | Hanken Grotesk 400 | 14 / 20          | —        | Texto de apoyo, ayuda de campo                      |
...
| `data-md` _(añadido)_  | JetBrains Mono 500 | 16 / 24          | 0        | **Valores medidos y numéricos en grillas**          |
...
- Sub-navegación por _bottom sheet_, no por menús desplegables.
...
6. Jerarquía de encabezados sin saltos; una sola `<h1>` por vista; _landmarks_ `header`/`nav`/`main`/`footer`.
