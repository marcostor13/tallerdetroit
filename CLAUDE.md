...
5. **Coolify con dos recursos**: push a `develop` → despliegue a _develop_; push a `main` → despliegue a _producción_.
...
- NestJS: un módulo por dominio (ver §15.3 de la especificación). `class-validator` en todos los DTOs.
  Los _guards_ de RBAC se declaran con `@Permissions()`, nunca con `if (user.role === ...)` disperso.
